...
public class SellerMenu {
    ...
    public void show(String sellerId, Scanner scanner) {
        ...
        while (!back) {
            ...
            System.out.println("| 2) View my actions               |");
            System.out.println("| 3) View verified credits         |");
            System.out.println("| 4) List credits for sale         |");
            System.out.println("| 5) View my listings              |");
            System.out.println("| 6) View remaining/unsold credits |");
            System.out.println("| 0) Logout                        |");
            System.out.println("+----------------------------------+");
            System.out.print("Select an option: ");
            ...
            switch (choice) {
                ...
                case "4" -> listCreditsForSale(sellerId, scanner);
                case "5" -> viewListings(sellerId);
                case "6" -> viewUnsoldCredits(sellerId);
                case "0" -> back = true;
                default -> System.out.println("⚠ Invalid option.");
            }
        }
    ...
    private void submitAction(String sellerId, Scanner scanner) {
        ...
        System.out.print("Description: ");
        String description = scanner.nextLine();
        System.out.print("Proof file path (optional): ");
        String proof = scanner.nextLine();

        SustainabilityAction action =
                sellerController.submitAction(sellerId, description, proof.isBlank() ? null : proof);

        System.out.println("✅ Action submitted with ID: " + action.getId());
    }
    ...
    private void viewActions(String sellerId) {
        ...
        for (SustainabilityAction a : actions) {
            ...
            System.out.println("  Description: " + a.getDescription());
            System.out.println("  Status: " + a.getStatus());
            System.out.println("  Admin note: " + a.getAdminReviewNote());
            System.out.println("----------------------------");
        }
    }
    ...
    private void viewVerifiedCredits(String sellerId) {
        List<CarbonCredit> credits = sellerController.getVerifiedCredits(sellerId);
        System.out.println();
        System.out.println("------ Verified Credits -----");
        if (credits.isEmpty()) {
            System.out.println("No verified credits.");
            return;
        }
    ...
    private void listCreditsForSale(String sellerId, Scanner scanner) {
        List<CarbonCredit> credits = sellerController.getVerifiedCredits(sellerId);
        if (credits.isEmpty()) {
            System.out.println("No verified credits to list.");
            return;
        }

        viewVerifiedCredits(sellerId);
        System.out.print("Credit ID to list (blank to cancel): ");
        String creditId = scanner.nextLine().trim();
        if (creditId.isEmpty()) {
            return;
        }
        ...
        try {
            CreditListing listing =
                    sellerController.listCredits(sellerId, creditId, quantity, price);
    ...
    private void viewUnsoldCredits(String sellerId) {
        List<CarbonCredit> credits = sellerController.getVerifiedCredits(sellerId);
        System.out.println();
        System.out.println("------ Remaining / Unsold ------");
        if (credits.isEmpty()) {
            System.out.println("No credits in non-sold state.");
            return;
        }
        for (CarbonCredit c : credits) {
